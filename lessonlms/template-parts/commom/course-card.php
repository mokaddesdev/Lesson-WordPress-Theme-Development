...
$stats              = lessonlms_get_review_stats($single_post_id);
...
$course_product     = get_post_meta( $single_post_id, '_course_product', true );
...
if (! empty( $user_enrollments ) && is_array( $user_enrollments ) ) {
    foreach ( $user_enrollments as $enroll ) {
        ...
        if ( $user_enroll_id == $single_post_id ) {
            $is_enroll = true;
            break;
        }
    }
...
if (! empty($user_wishlists) && is_array($user_wishlists)) {
    foreach ($user_wishlists as $wishlist) {
        if (isset($wishlist['course_id']) && intval($wishlist['course_id']) === $single_post_id) {
...
$image_html     = '';
if (has_post_thumbnail($single_post_id)) {
            ...
            <div class="wishlist-btn">
                <?php if ($allready_has_wishlist === false) : ?>
                    <form method="post" class="add-to-wishlist-form">
                        ...
                        <input type="hidden" name="course_id" value="<?php echo esc_attr($single_post_id); ?>">
